...
export function timelineIconFor(title: string): TimelineIcon {
	...
	if (/ceremon|iglesia|parroquia|misa|boda|votos|civil/.test(t))
		return "ceremonia"
	...
	if (/fiesta|baile|bailar|m[uú]sica|dj|hora loca|celebraci/.test(t))
		return "fiesta"
	return "momento"
}
...
export function parseNotes(text: string | null | undefined): GuestNote[] {
	return blocks(text).map((b) => {
		if (b.length >= 2)
			return { title: b[0].replace(/:$/, ""), text: b.slice(1).join("\n") }
...
export function buildWeddingContent(row: Wedding | null): WeddingContent {
	...
	return {
		...
		quote: quoteText
			? { text: quoteText, attribution: clean(row?.quoteAttribution) }
			: null,
		ceremony,
		reception,
		timeline,
		...
		media: {
			...
			inviteImageUrl: clean(row?.coupleImageUrl) ?? LANDING_ASSETS.inviteImage,
...
export function landingSections(
	...
	if (c.timeline.length || c.dressCode || c.notes.length)
		s.push({ id: "detalles", label: "Detalles" })
