  ...
  return (
    ...
    >
      <div aria-hidden className="pointer-events-none absolute inset-0 -z-10">
        <Parallax
          speed={0.12}
          className="absolute -right-14 top-6 w-32 text-(--color-sage) opacity-35 sm:w-48"
        >
        ...
        <Parallax
          speed={0.16}
          className="absolute -left-10 top-1/3 w-28 text-(--color-sage) opacity-30 sm:w-40"
        >
          <div className="rotate-150">
        ...
        <Parallax speed={0.2} className="absolute inset-0" innerClassName="relative h-full w-full">
          <Leaf className="landing-drift-a absolute left-[8%] top-[22%] w-3 rotate-45 text-(--color-sage) opacity-40" />
          <Leaf className="landing-drift-c absolute bottom-[18%] right-[10%] w-3.5 -rotate-12 text-(--color-gold-deep) opacity-40" />
        </Parallax>
      </div>
      ...
      <div className="mx-auto max-w-2xl">
        ...
        {welcome ? (
          <Reveal variant="blur" delay={120}>
            <p className="mt-10 whitespace-pre-line text-center font-display text-[1.3rem] font-light leading-[1.7] text-(--color-foreground)/90 sm:text-[1.45rem]">
              {welcome}
            </p>
          </Reveal>
        ) : null}
        ...
        {couple.parents.length ? (
          <Reveal delay={160}>
            <div className="landing-parents mt-10 grid gap-6 border-y border-(--color-border) py-7 sm:grid-cols-2">
              {couple.parents.map((group) => (
                <div key={group.line} className="text-center">
                  <p className="text-[0.62rem] uppercase tracking-[0.3em] indent-[0.3em] text-(--color-accent)">
                    {group.line}
                  </p>
                  <ul className="mt-2 space-y-0.5">
                    {group.names.map((name, i) => (
                      <li
                        key={i}
                        className="font-display text-lg font-light italic text-(--color-foreground)/85"
                      >
                        {name}
                      </li>
                    ))}
        ...
        <Reveal variant="scale" delay={120} className="mt-14 flex justify-center pr-3 sm:pr-6">
          <Tilt max={4} className="relative w-[min(70vw,20rem)]">
            <div
              aria-hidden
              className="absolute -inset-2 -z-10 translate-x-1.5 -translate-y-1.5 rounded-t-[999px] border border-gold-deep/70 sm:-inset-2.5 sm:translate-x-3 sm:-translate-y-3"
            />
            <div
              aria-hidden
              className="absolute -inset-4 -z-20 translate-x-3 -translate-y-3 rounded-t-[999px] border border-gold-deep/30 sm:-inset-5 sm:translate-x-6 sm:-translate-y-6"
            />
            <div className="landing-arch overflow-hidden rounded-t-[999px] border border-(--color-border) bg-(--color-muted) shadow-(--shadow-warm)">
              <CoupleImage
        ...
        {story.length ? (
          <div className="mx-auto mt-14 max-w-xl">
            <Reveal variant="line">
              <p className="text-center text-[0.65rem] uppercase tracking-[0.36em] indent-[0.36em] text-(--color-accent)">
                Nuestra historia
              </p>
            </Reveal>
            {story.map((p, i) => (
              <Reveal key={i} variant="blur" delay={100 + i * 80}>
                <p className="mt-5 text-center font-display text-[1.15rem] font-light leading-[1.8] text-(--color-foreground)/85 sm:text-[1.2rem]">
                  {p}
                </p>
              </Reveal>
            ))}
        ...
        {quote ? (
          <Reveal variant="blur" delay={120} className="mt-16 text-center">
            <div className="mx-auto w-24 text-gold-deep/70">
              <Ornament />
            </div>
            <blockquote className="mt-6">
              <p
                className="text-[clamp(1.6rem,5.4vw,2.4rem)] leading-[1.4] text-(--color-accent) "
                style={{ fontFamily: 'var(--font-serif-stack)' }}
              >
                {quote.text}
              </p>
              {quote.attribution ? (
                <footer className="mt-5 text-[0.65rem] uppercase tracking-[0.36em] indent-[0.36em] text-(--color-muted-foreground)">
                  {quote.attribution}
                </footer>
              ) : null}
            </blockquote>
